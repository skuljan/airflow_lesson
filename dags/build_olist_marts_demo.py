...
def get_client():
    ...
    """
    # FIX: импорт внутри функции — чтобы DAG не был Broken,
    # если зависимость временно не установлена
...
def run_sql(sql: str) -> None:
    """
    Выполняем SQL по statement'ам (упрощённо).
    """
    client = get_client()
...
(
  ...
  -- FIX: храним средний review как Float
...
SELECT
  ...
  avgIf(avg_review_score, avg_review_score IS NOT NULL) AS avg_review_score,

  avgIf(toFloat64(delivery_days), delivery_days IS NOT NULL) AS avg_delivery_days,

  -- FIX: гарантируем float-деление
  if(
    countIf(order_status = 'delivered' AND delivery_delay_days IS NOT NULL) = 0,
    NULL,
    toFloat64(countIf(order_status = 'delivered' AND delivery_delay_days > 0))
...
with DAG(
    ...
    schedule=None,          # руками для отладки; позже можно @daily
    catchup=False,
    max_active_runs=1,      # FIX: защита от параллельных запусков
